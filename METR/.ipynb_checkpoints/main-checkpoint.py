...
def get_args_parser():
    ...
    parser.add_argument('--is_prompt_indicator', action='store_true')
    parser.add_argument('--is_embedding_align', action='store_true')
    parser.add_argument('--num_classes_for_CEM', default=80, type=int, choices=[0, 20, 80])
    parser.add_argument('--eval_with_agnostic', action='store_true')
    parser.add_argument('--use_val_for_training', action='store_true')
    parser.add_argument('--use_plain_CEM', action='store_true')
...
def build_model_main(args):
    ...
    model, criterion, criterion_for_encoder, postprocessors = build_func(args)
...
def main(args):
    ...
    # build model
    model, criterion, criterion_for_encoder, postprocessors = build_model_main(args)
...
#         args.resume = os.path.join(args.output_dir, 'checkpoint.pth')
    ...
    if args.eval:
        ...
        test_stats, coco_evaluator = evaluate(model, criterion, criterion_for_encoder, postprocessors,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, criterion_for_encoder, data_loader_train, optimizer, device, epoch,
        ...
        test_stats, coco_evaluator = evaluate(
            model, criterion, criterion_for_encoder, postprocessors, data_loader_val, base_ds, device, args.output_dir,
...
if __name__ == '__main__':
    ...
    args.output_dir = 'logs/'+args.config_file.split('/')[-1][:-3]
    if args.output_dir:
