...
def build_backbone(args):
    ...
    elif args.backbone in ['vit_fpn']:
        backbone = ViT_FPN(img_size=args.img_size, patch_size=args.patch_size, embed_dim=args.embed_dim, in_chans=3, out_indices=args.out_indices,
        depth=args.depth, num_heads=args.num_heads, init_values=args.init_values, mlp_ratio=args.mlp_ratio, drop_path_rate=args.drop_path_rate,
        use_abs_pos_emb=args.use_abs_pos_emb, use_sincos_pos_emb=args.use_sincos_pos_emb, use_rel_pos_bias=args.use_rel_pos_bias,
        qkv_bias=args.qkv_bias, use_checkpoint=args.use_checkpoint)
        bb_num_channels = backbone.num_features[4 - len(return_interm_indices):]
        if args.backbone_dir is not None:
            backbone.init_weights(args.backbone_dir)
    else:
