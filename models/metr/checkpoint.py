...
def load_checkpoint(model,
    ...
    # for COTR, load pre-trained weights
    ...
    for key in list(state_dict.keys()):
        if key == 'patch_embed.proj.weight':
            tmp_patch_embed = state_dict['patch_embed.proj.weight']
            new_patch_embed = F.interpolate(tmp_patch_embed, size=(16, 16), mode="bicubic", align_corners=False)
            state_dict['patch_embed.proj.weight'] = new_patch_embed
    
    # for distill net, load pre-trained weights
    if all_keys[-1].startswith('s_net') or all_keys[-1].startswith('Connectors') or all_keys[-1].startswith('t_net'):
        # NOTE: remove all decoder keys
        all_keys = [key for key in all_keys if key.startswith('s_net.')]
        for key in all_keys:
            new_key = key.replace('s_net.','')
            state_dict[new_key] = state_dict[key]
            state_dict.pop(key)
            
        for key in list(state_dict.keys()):
            if key.startswith('t_net.') or key.startswith('Connectors.'):
                # print("key:", key)
                state_dict.pop(key)
    ...
    print("############before len:", len(state_dict))
    if "rel_pos_bias.relative_position_bias_table" in state_dict:
        if rank == 0:
...
#
#        state_dict.pop("rel_pos_bias.relative_position_bias_table")
    print("############after len:", len(state_dict))
    ...
    for key in all_keys:
        ...
        if "relative_position_bias_table" in key and key in model.state_dict():
            ...
            print("################rel_pos_bias size:", rel_pos_bias.shape)
            src_num_pos, num_attn_heads = rel_pos_bias.size()
            dst_num_pos, _ = model.state_dict()[key].size()
            print("################dst_num_pos:", dst_num_pos)
            dst_patch_shape = model.patch_embed.patch_shape
            print("################patch_embed shape:", model.patch_embed.patch_shape)
    ...
    if 'pos_embed' in state_dict:
        ...
        print("############load before:", pos_embed_checkpoint.shape)
        ...
        if orig_size != new_size_h or orig_size != new_size_w:
            ...
            print("############extra_token, num_extra_tokens:", extra_tokens.shape, num_extra_tokens)
        ...
        print("#############load after:", new_pos_embed.shape, new_pos_embed)
