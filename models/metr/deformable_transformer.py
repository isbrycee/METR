...
from .prompt_indicator_plain import PromptIndicator_plain
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.class_topk = args.retention_policy_train_max_classes
        self.plain = args.use_plain_CEM
        ...
        if self.is_prompt_indicator:
            if self.plain:
                self.prompt_indicator = PromptIndicator_plain(args)
            else:
                self.prompt_indicator = PromptIndicator(args)
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points, add_channel_attention=add_channel_attention, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type, is_embedding_align=self.is_embedding_align)
        else:
        ...
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          is_embedding_align=self.is_embedding_align)
        ...
        if self.is_prompt_indicator:
            if args.text_embed_type == 'clip':
                self.prompt_indicator._init_class_prompts(args)
            else:
                assert args.text_embed_type == 'none'
    ...
    def _reset_parameters(self):
        ...
        if self.is_embedding_align:
            from .ops.modules.ms_deform_attn_metr import MSDeformAttn
        else:
    ...
    def init_ref_points(self, use_num_queries):
        ...
        if self.random_refpoints_xy:  # false
            self.refpoint_embed.weight.data[:, :2].uniform_(0,1)
            self.refpoint_embed.weight.data[:, :2] = inverse_sigmoid(self.refpoint_embed.weight.data[:, :2])
            self.refpoint_embed.weight.data[:, :2].requires_grad = False

    def get_query_with_text_indicator(self, class_vector, tgt):

        bs = tgt.shape[0]
        cs_all, c = class_vector.shape
        if self.is_embedding_align:
            tgt = tgt.repeat(cs_all // bs, 1, 1) + class_vector.view(cs_all, 1, c)
        else:
            tgt = tgt.transpose(0, 1)
            class_vector = torch.cat(class_vector.unsqueeze(1).split(cs_all // bs, dim=0), dim=1)
            class_vector = class_vector.repeat_interleave(self.num_queries // (cs_all // bs), 0)
            tgt = torch.cat([class_vector + i for i in tgt.split(self.num_queries, dim=0)], 0)

        return tgt

    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, targets=None):
        """
        Input:
        ...
        if self.is_prompt_indicator:
            outputs, loss_dict = {}, {}
            if self.plain:
                class_prompts, select_id = self.prompt_indicator(memory.transpose(0, 1), targets, kwargs={})

                additional_object_inputs = dict(
                    bs_idx=torch.zeros(self.class_topk, dtype=torch.long).cuda(),
                    cls_idx=torch.tensor(select_id).cuda(),
                    class_vector=class_prompts[select_id].cuda(),  # 20, 256
                    original_tgt_class=class_prompts.unsqueeze(0).cuda(),  # 1,80,256
                )
                class_vector = additional_object_inputs['class_vector']  # 20, 256
                cls_outputs = {}
                cls_outputs['original_tgt_class'] = additional_object_inputs['original_tgt_class']  # 1,80,256
                loss_dict = None
            else:
                cls_outputs, cls_loss_dict = self.prompt_indicator(memory.transpose(0, 1), mask_flatten, targets=targets, kwargs={})
                ...
                additional_object_inputs = dict(
                    ...
                    class_vector=outputs["tgt_class"],  # 20, 256
                    original_tgt_class=outputs["original_tgt_class"],
                )
                class_vector = additional_object_inputs['class_vector']
        else:
            loss_dict = None
            additional_object_inputs = None
            class_vector = None
        ...
        if self.two_stage_type =='standard':
            ...
            if self.is_prompt_indicator:
                if self.num_classes_for_CEM == 20:
                    enc_outputs_class_unselected = torch.bmm(self.enc_out_class_embed(output_memory), class_vector.reshape(bs, class_vector.shape[0]//bs, -1).transpose(1, 2) * W)
                elif self.num_classes_for_CEM == 80:
                    enc_outputs_class_unselected = torch.bmm(self.enc_out_class_embed(output_memory), cls_outputs['original_tgt_class'].transpose(1, 2) * W)
            else:
            ...
            if self.is_prompt_indicator:
                tgt_ = self.get_query_with_text_indicator(class_vector, tgt_)
                if not self.is_embedding_align:
                    tgt_ = tgt_.transpose(0, 1)

            if refpoint_embed is not None: 
                refpoint_embed=torch.cat([refpoint_embed,refpoint_embed_],dim=1)
                if self.is_embedding_align:
                    tgt = tgt.repeat(self.class_topk,1,1)
                tgt=torch.cat([tgt,tgt_],dim=1) # tgt[0] not equal tgt[1], note here
            else:
            ...
            else:  # no cdn, one stage with here
                refpoint_embed,tgt=refpoint_embed_,tgt_

            if self.num_patterns > 0: # False
        ...
        hs, references = self.decoder(
                ...
                is_embedding_align = self.is_embedding_align,
                is_prompt_indicator = self.is_prompt_indicator,
                class_topk = self.class_topk,
        ...
        if self.two_stage_type == 'standard':
            if self.two_stage_keep_all_tokens:
                ...
                hs_enc = tgt_undetach.unsqueeze(0) # here; tgt_undetach.shape torch.Size([1, 900, 256])
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, loss_dict, class_vector, additional_object_inputs
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                is_embedding_align = False,
                is_prompt_indicator = False,
                class_topk = 10,
                additional_info = {}
                ):
        """
        ...
        """
        bs = refpoints_unsigmoid.shape[1]

        if is_embedding_align:
            refpoints_unsigmoid = refpoints_unsigmoid.repeat(1,class_topk,1)
            valid_ratios = valid_ratios.repeat(class_topk,1,1)
        if is_prompt_indicator:
            class_vector = additional_info.pop("class_vector", None)
            bs_idx = additional_info["bs_idx"]
            cls_idx = additional_info["cls_idx"]
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.training and self.decoder_query_perturber is not None and layer_id != 0: #no
                reference_points = self.decoder_query_perturber(reference_points)
            if self.deformable_decoder:
                if reference_points.shape[-1] == 4: # here                    
            ...
            if self.bbox_embed is not None:
                ...
                # select # ref points
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1: # no
                ...
                if self.use_detached_boxes_dec_out:
                    ...
                    ref_points.append(new_reference_points) # here
                    
                # ref_points.append(reference_points) # note here, use fixed reference_points
                
            intermediate.append(self.norm(output))  # note here have another norm
            if self.dec_layer_number is not None and layer_id != self.num_layers - 1: # no
                if nq_now != select_number:
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if use_deformable_box_attn:
            ...
            self.cross_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points) # here
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
    ...
    def forward_ca(self,
        ...
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
                               tgt_reference_points.transpose(0, 1).contiguous(),
                               memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index, memory_key_padding_mask, cs_batch = cs_batch).transpose(0, 1)
                ...
                tgt = self.forward_ca(tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask, cs_batch)
            elif funcname == 'sa': # here
